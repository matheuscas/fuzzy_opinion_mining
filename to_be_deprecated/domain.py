...
class Cornell(object):
	...
	def create_documents_model(self, weight="TFIDF"):
		documents = []
		weights = {"TFIDF":v.TFIDF, "TF":v.TF, "IG":v.TF, "BINARY":v.BINARY}

		for d in self.positives + self.negatives:
			documents.append(v.Document(d.raw_text))

		self.model = v.Model(documents=documents, weight=weights[weight])
